...
const Navbar = () => {
    ...
    return (
        <div className={`${styles.container} bl ${isVisible ? styles.visible : styles.hidden}`}>
            ...
            {isSidebarVisible && (
                <ul className={`${styles.sideBar} ${isClosing ? styles.close : styles.open}`}>
                    <div className={styles.closeSidebar} onClick={toggleSidebar}>
                        <img src={closeIcon} alt="" />
                    </div>
                    <li className={styles.navLink}><Link to='/Inscripciones' onClick={handleSidebarClick(toggleSidebar)}>Inscripciones</Link></li>
