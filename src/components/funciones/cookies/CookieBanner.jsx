...
const CookieBanner = ({ activateTracking }) => {
  ...
  // Función para aceptar todas las cookies desde el banner
  ...
  // Al cerrar el modal sin guardar cambios, volvemos a mostrar el banner
  const handleCloseModal = () => {
    setShowCookieModal(false);
    setShowCookieBanner(true); // Mostrar el banner nuevamente
  };
  ...
  return (
    <>
          ...
          <div className={styles.banner}>
            <div className={styles.titulo}>
              <div className={styles.imgCont}>
                ...
                <img src={close} alt="Cerrar" onClick={handleCloseBanner} />
              </div>
              <small>
                Al hacer clic en “Aceptar”, aceptás el almacenamiento de cookies en tu
                dispositivo para mejorar la navegación del sitio, analizar el uso del
                sitio y ayudar en nuestros esfuerzos de marketing. {''}
      ...
      {/* Se muestra el modal de configuración cuando showCookieModal es true */}
      {showCookieModal && (
          ...
          <div className={styles.modalContent}>
            ...
            <div className={styles.privacy}>
              {Object.keys(cookieInfo).map((category) => (
                <div
                  key={category}
                  className={`${styles.cookieSection} ${selectedCategory === category ? styles.selected : ""}`}
                >
                  <div
                    className={`${styles.cookieTitle} ${selectedCategory === category ? styles.selected : ""}`}
                    onClick={() => setSelectedCategory(category)}
                    onKeyDown={(e) =>
                      (e.key === "Enter" || e.key === " ") && setSelectedCategory(category)
                    }
                    tabIndex="0"
                  >
                    <h4>{category}</h4>
                    {selectedCategory === category &&
                      ...
                      category !== "Estrictamente necesarias" && (
                        <select
                          ...
                          value={preferences[getPreferenceKey(category)] ? "activadas" : "desactivadas"}
                          onChange={(e) => {
                  ...
                  <div className={`${styles.cookiesText} ${selectedCategory === category ? styles.active : ""}`}>
                    <p>{cookieInfo[category].description}</p>
                    {cookieInfo[category].cookies.length > 0 && (
                      <p>
                        <strong>Tipos:</strong> {cookieInfo[category].cookies.join(", ")}
                      </p>
                    )}
                  </div>
                </div>
            ...
            <div className={styles.save}>
              <button className="btn-cta grad2 bl" onClick={handleSave}>
                Confirmar cambios
              </button>
            </div>
          </div>
